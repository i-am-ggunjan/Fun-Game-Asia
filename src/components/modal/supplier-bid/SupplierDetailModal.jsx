...
const SupplierDetailModal = ({ data = [{}, {}], title, handleModalClose, searchDate }) => {
    ...
    const columns = [
        ...
        { name: 'City', selector: row => row?.city },
        {
            name: 'Action', cell: row => <div onClick={() => {
                navigate(`/supplier/supplier-bid/purchase-order/${row?._id}`, { state: { stateData: { name: row?.businessName, searchDate } } })
                handleModalClose()
                    ...
                    <Grid item md={5} xs={12} sx={{ padding: "20px", backgroundColor: Color.greyBg, borderRadius: "10px" }}>
                        ...
                        <div style={{ backgroundColor: "#FFF", padding: "20px", borderRadius: "10px", display: "flex", flexDirection: "column", gap: '15px' }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                                <div onClick={() => navigate(`/supplier/supplier-bid/purchase-order/${filteredData[0]?._id}`, { state: { stateData: { name: filteredData[0]?.businessName, searchDate } } })} style={{ display: "flex", justifyContent: "center" }}>
