...
const ItemCategory = () => {
  ...
  const { itemCategoryData } = useSelector(state => state?.masterReducer);
  ...
  const handleSubmit = () => {
    ...
    if (mode == 'Edit') {
      ...
      //! Dispatching API For Update Item Category
      dispatch(MasterActions.updateItemCategory(payload));
      ...
      //! Dispatching API For Create Item Category
      dispatch(MasterActions.createItemCategory(payload));
    }
  }

  //* Datatable Columns
  const columns = [
    { name: 'S.No.', selector: row => itemCategoryData?.indexOf(row) + 1 },
    ...
    { name: 'Image', selector: row => <img src={`data:image/png;base64, ${row?.image}`} height={'50px'} width={'50px'} /> },
    {
      ...
      cell: row => (
        <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
          ...
          <div onClick={() => dispatch(MasterActions.deleteItemCategory({ id: row?._id }))} style={{ cursor: "pointer" }}><DeleteSvg /></div>
        </div>
      ),
      right: true
    },
  ...
  useEffect(() => {
    //! Dispatching API For Getting Item Category
    dispatch(MasterActions.getItemCategory());
  }, []);

  return (
    <div style={{ padding: "10px" }}>
      <RouteHeader title={'Category'} />
      ...
      <div style={{ backgroundColor: Color.white, padding: "20px", borderRadius: "10px" }}>
        ...
        <div style={{ backgroundColor: Color.white, padding: "20px", borderRadius: "10px", boxShadow: "1px 1px 5px grey" }}>
          <DatatableHeading title={'Category'} data={itemCategoryData} />
          <DatatableWithSearch data={itemCategoryData} columns={columns} />
        </div>
      </div>
    </div>
  )
