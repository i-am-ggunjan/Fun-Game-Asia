...
const SaleArea = () => {
  ...
  const { saleAreaData } = useSelector(state => state?.masterReducer);
  ...
  //* Datatable Columns
  const columns = [
    { name: 'S.No.', selector: row => saleAreaData?.indexOf(row) + 1 },
    { name: 'Name', selector: row => row?.name },
    {
      ...
      cell: row => (
        <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
          ...
          <div onClick={() => dispatch(MasterActions.deleteSaleArea({ id: row?._id }))} style={{ cursor: "pointer" }}><DeleteSvg /></div>
        </div>
      ),
      right: true
    },
  ...
  useEffect(() => {
    //! Dispatching API For Getting Sale Area
    dispatch(MasterActions.getSaleArea());
  }, []);

  return (
    <div style={{ padding: "10px" }}>
      <RouteHeader title={'Sale Area'} />
      ...
      <div style={{ backgroundColor: Color.white, padding: "20px", borderRadius: "10px" }}>
        ...
        <div style={{ backgroundColor: Color.white, padding: "20px", borderRadius: "10px", boxShadow: "1px 1px 5px grey" }}>
          <DatatableHeading title={'Sale Area'} data={saleAreaData} />
          <DatatableWithSearch data={saleAreaData} columns={columns} />
        </div>
      </div>
    </div>
  )
