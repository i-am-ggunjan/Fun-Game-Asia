...
const Category = () => {
    ...
    const supportCategoryData = [{ title: 'Cat One' }, { title: 'Cat Two' }];
    ...
    //* Datatable Columns
    const columns = [
        { name: 'S.No.', selector: row => supportCategoryData?.indexOf(row) + 1 },
        { name: 'Title', selector: row => row?.title },
        {
            ...
            cell: row => (
                <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
                    <div
                        onClick={() => navigate(`/setting/support/category/${row?.title?.toLowerCase()?.split(' ').join('')}`, {
                            state: { stateData: row },
                        })}
                        style={{ cursor: "pointer" }}><ViewSvg /></div>
        ...
        <div style={{ padding: "10px" }}>
            ...
            <div style={{ backgroundColor: Color.white, padding: "20px", borderRadius: "10px" }}>
                ...
                <div style={{ backgroundColor: Color.white, padding: "20px", borderRadius: "10px", boxShadow: "1px 1px 5px grey" }}>
                    <DatatableHeading title={'Category'} data={supportCategoryData} />
                    <DatatableWithSearch data={supportCategoryData} columns={columns} />
                </div>
            </div>
        </div>
    )
